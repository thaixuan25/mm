...
def test_coeff_from_three_bytes_accepts_below_q():
    z = CoeffFromThreeBytes(0xFF, 0xFF, 0x7F)
    assert z is None
    z2 = CoeffFromThreeBytes(0x00, 0x00, 0x00)
    assert z2 == 0
    z3 = CoeffFromThreeBytes(0x01, 0x00, 0x00)
    assert z3 == 1
...
def test_expand_a_dimensions_and_determinism():
    rho = b"\x01" * 32
    A1 = ExpandA(rho, ML_DSA_44_PARAMS)
    A2 = ExpandA(rho, ML_DSA_44_PARAMS)
...
def test_expand_s_dimensions_and_range():
    rho = b"\x02" * 64
    s1, s2 = ExpandS(rho, ML_DSA_65_PARAMS)
...
def test_expand_mask_range_and_dimensions():
    rho_pp = b"\x03" * 64
    y = ExpandMask(rho_pp, 0, ML_DSA_87_PARAMS)
...
def test_sample_in_ball_deterministic_for_same_seed():
    ...
    c1 = SampleInBall(rho, ML_DSA_65_PARAMS)
    c2 = SampleInBall(rho, ML_DSA_65_PARAMS)
    assert c1 == c2


def test_sampling_rejects_wrong_seed_lengths():
    with pytest.raises(ValueError):
        ExpandA(b"\x00" * 31, ML_DSA_44_PARAMS)
    with pytest.raises(ValueError):
        ExpandS(b"\x00" * 63, ML_DSA_65_PARAMS)
    with pytest.raises(ValueError):
        ExpandMask(b"\x00" * 63, 0, ML_DSA_87_PARAMS)
    with pytest.raises(ValueError):
        SampleInBall(b"\x00" * (ML_DSA_65_PARAMS.c_tilde_bytes - 1), ML_DSA_65_PARAMS)
