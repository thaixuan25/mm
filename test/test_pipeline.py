...
@pytest.mark.parametrize("scheme", PARAMS)
def test_prehash_sign_verify_round_trip(scheme: ML_DSA):
    pk, sk = scheme.keygen(xi=b"\x0c" * 32)
    msg = b"message large enough to be pre-hashed"
    sig = scheme.sign(sk, msg, ctx=b"hash-mode", deterministic=True, pre_hash="sha256")
    assert scheme.verify(pk, msg, sig, ctx=b"hash-mode", pre_hash="sha256") is True
    assert scheme.verify(pk, msg, sig, ctx=b"hash-mode") is False
    assert scheme.verify(pk, msg + b"!", sig, ctx=b"hash-mode", pre_hash="sha256") is False


@pytest.mark.parametrize("scheme", PARAMS)
def test_prehash_aliases_match_for_sign_and_verify(scheme: ML_DSA):
    pk, sk = scheme.keygen(xi=b"\x0d" * 32)
    msg = b"prehash alias"
    sig1 = scheme.sign(sk, msg, deterministic=True, pre_hash="sha256")
    sig2 = scheme.sign(sk, msg, deterministic=True, pre_hash="SHA2-256")
    assert sig1 == sig2
    assert scheme.verify(pk, msg, sig1, pre_hash="SHA-256") is True


def test_invalid_prehash_name_rejected_without_changing_pure_api():
    pk, sk = ML_DSA_44.keygen(xi=b"\x0e" * 32)
    msg = b"bad hash name"
    sig = ML_DSA_44.sign(sk, msg, deterministic=True)
    assert ML_DSA_44.verify(pk, msg, sig) is True
    with pytest.raises(ValueError):
        ML_DSA_44.sign(sk, msg, deterministic=True, pre_hash="not-a-hash")
    assert ML_DSA_44.verify(pk, msg, sig, pre_hash="not-a-hash") is False
