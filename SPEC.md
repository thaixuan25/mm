...
- HashML-DSA dùng `pre_hash` keyword-only ở API cấp cao; `None` giữ mode `pure`, còn các tên như `sha256`, `sha512`, `sha3_256`, `shake128`, `shake256` tạo `M' = 0x01 || |ctx| || ctx || OID(PH) || PH(M)`.
...
- Tiền xử lý thông điệp tập trung ở `ml_dsa/prehash.py`: `pure` dùng domain separator `0x00`, HashML-DSA dùng `0x01`.
